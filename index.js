...
let batans = global.batrei[global.batrei.length - 1]
  ...
  break
case 'editanime': 
sayo = fs.readFileSync('./assets/edits/editanime.mp4') //LOCAL DO /VÍDEO
				/*case 'menu':
				if (!isRegistrar) return reply(mess.registro)//PEDI O REGISTRO
...
if (!isRegistrar) return reply(mess.registro)//PEDI O REGISTRO
				...
				if (!isRegistrar) return reply(mess.registro)//PEDI O REGISTRO
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* KEN\n*YOUTUBE:* https://youtube.com/channel/UCxz0aBAu-geMp5zTwessHhg\n*WPP:* wa.me/+557999076521\n\n\nEspero que tenham gostado do bot 🥵'})
					  client.sendMessage(from, 'Ctt do meu dono ai Wa.me/557999076521 , pfv n flode',MessageType.text, { quoted: freply} )
                    break
...
break
				...
				if (!isRegistrar) return reply(mess.registro)//PEDI O REGISTRO
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `[ 𝐓𝐫𝐚𝐧𝐬𝐦𝐢𝐬𝐬𝐚̃𝐨 ♞͜͡🐊ঔৣ͜͡𝙆ΞИ☂︎™ ]\n\n${body.slice(4)}`})
						}
						reply('Transmissao enviada')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `[ 𝐓𝐫𝐚𝐬𝐦𝐢𝐬𝐚̃𝐨 ♞͜͡🐊ঔৣ͜͡𝙆ΞИ☂︎™ ]\n\n${body.slice(4)}`)
						}
...
if (!isRegistrar) return reply(mess.registro)//PEDI O REGISTRO
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ 𝐀𝐯𝐢𝐬𝐨 ♞͜͡🐊ঔৣ͜͡𝙆ΞИ☂︎™ ]\n\n${body.slice(7)}`)
						}
						reply('Tm enviada com sucesso')
					}
					break
...
if (!isRegistrar) return reply(mess.registro)//PEDI O REGISTRO
			    ...
			    buffer = await getBuffer(`https://api-exteam.herokuapp.com/api/card-spotify?titulo=${encodeURIComponent(titulo)}&author=${encodeURIComponent(anu.result.source)}&album=KENBOT&capa=${anu.result.thumbnail}`)
				...
				foto = await getBuffer(`https://api-exteam.herokuapp.com/api/card-spotify?titulo=${encodeURIComponent(anu.result.title)}&author=${encodeURIComponent(anu.result.source)}&album=KEN-BOT&capa=${anu.result.thumbnail}`)
